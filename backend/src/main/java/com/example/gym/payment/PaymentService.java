...
public class PaymentService {
	...
	private final PaymentNotificationListener eventPublisher;
	...
	public PaymentService(PaymentRepository paymentRepository, MembershipRepository membershipRepository,
			...
			DeviceAuthorizationService deviceAuthorizationService, AuditService auditService,
			PaymentNotificationListener eventPublisher) {
	...
	public Page<Payment> list(Long tenantId, Pageable pageable, LocalDate from, LocalDate to) {
		...
		if (from == null || to == null) {
			throw CommonExceptions.badRequest("from and to dates must be provided together");
		}
		...
		return paymentRepository.findByTenantIdAndPaidOnBetweenOrderByPaidOnDescIdDesc(tenantId, from, to, pageable);
	}
	...
	public Payment record(RecordPayment request, Long tenantId) {
		...
		if (request.amount() == null || request.amount().signum() <= 0) {
			...
			throw CommonExceptions.badRequest("Payment amount must be greater than zero");
		}
		...
		Payment payment = new Payment(tenantId, member.getId(), membership == null ? null : membership.getId(),
		...
		auditService.record(AuditActions.PAYMENT_RECORDED, AuditActions.RESULT_SUCCESS, "Payment", saved.getPublicId(),
				Map.of("memberId", member.getPublicId(), "amount", saved.getAmount().toPlainString(), "currency",
						currency));

		/*
		 * Publish only after the payment has been successfully persisted.
		 *
		 * PaymentNotificationListener will receive this AFTER_COMMIT.
		 */
		eventPublisher.onPaymentChanged(
				new PaymentChangedEvent(tenantId, saved.getMemberId(), saved.getId(), PaymentEventType.RECORDED));
	...
	public Payment refund(String paymentPublicId, Long tenantId) {
		...
		if (payment.getMembershipId() != null) {
			...
			membershipRepository.findById(payment.getMembershipId()).ifPresent(this::recomputeMembershipPaymentStatus);
		}

		auditService.record(AuditActions.PAYMENT_REFUNDED, AuditActions.RESULT_SUCCESS, "Payment", saved.getPublicId(),
				null);

		eventPublisher.onPaymentChanged(
				new PaymentChangedEvent(tenantId, saved.getMemberId(), saved.getId(), PaymentEventType.REFUNDED));

		return saved;
	}

	/**
	 * Recomputes a membership's amountPaid + payment status from its COMPLETED
	 * payments.
	 */
	private void recomputeMembershipPaymentStatus(Membership membership) {
		BigDecimal paid = paymentRepository.findByMembershipIdAndStatus(membership.getId(), PaymentStatus.COMPLETED)
	...
	@Transactional(readOnly = true)
	public PaymentSummaryResponse summary(Long tenantId, LocalDate from, LocalDate to) {

		if (from.isAfter(to)) {
			throw CommonExceptions.badRequest("from date cannot be after to date");
		}

		BigDecimal total = paymentRepository.sumCompletedBetween(tenantId, from, to);

		long count = paymentRepository.countByTenantIdAndStatusAndPaidOnBetween(tenantId, PaymentStatus.COMPLETED, from,
				to);

		return new PaymentSummaryResponse(total == null ? BigDecimal.ZERO : total, count);
	}
}
